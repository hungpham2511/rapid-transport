...
# %% important area, choose 1000 samples
print ("Start sampling")
N_samples = 1000
c_mean = np.mean(c_vec, axis=0)
c_cov = np.cov(c_vec.T) * 20
# c_cov[0, 0] = 0.01
ws_thin = []
N_atempt = 0
while len(ws_thin) < N_samples:
    N_atempt += 1
    sample = np.random.multivariate_normal(c_mean, c_cov)
    if np.all(A.dot(sample) <= b):
        ws_thin.append(sample)
ws_thin = np.array(ws_thin)
print "Took {:d} attempts to generate {:d} samples".format(N_atempt, N_samples)
...
A, b = hull.get_halfspaces()
np.savez(os.path.join(db.get_contact_data_dir(), "nonprehensile_gripper_simplified_strategy9c.npz"), A=A, b=b)
